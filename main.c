...
void DisasVmIns(HWND hwnd) {
	...
	if (!(rws = (RWinStruct*)GetWindowLong(hwnd, 0))) return;

	ip = (unsigned int)vm_get_current_instruction(rws->vm);

	//нужно получить текущий размер инструкции(1, 7, 11)
...
RWinStruct *rws;
    ...
    switch (iMsg){
		...
		case WM_TIMER:
			...
			//функция проверяет по таймеру есть ли клавиша нажатая. если есть ставит ее(стрелочку) в регистр или 0 если нет
			//и меняет стейт машины на ОК
			//StepOver(hwnd);
