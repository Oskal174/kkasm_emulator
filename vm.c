...
#define		VM_SHR		19
#define		VM_LEA		20
//команды перехода(однооперандные)
#define		VM_JMP		21
#define		VM_JA		22
#define		VM_JAE		23
#define		VM_JB		24
...
// значения типов операндов(режимы адресации)
...
//
//запоминает код последней нажатой клавиши
//
void vm_setCurrentKey(vm_struct *vm, int keyCode) {
	currentKey = keyCode;
...
//
//Получение размера текущей инструкции
//
unsigned int vm_getCurrentInstSize(vm_instruction *ins) {
	unsigned int size = 0;
...
//--------------------
//
// Читает двойное слово из дескриптора
//
static unsigned int vm_get_dword(vm_struct *vm) {
...
//
// Обработчик инструкции rand
// Возвращает случайное значение в аргумент1, аргумент2 - верхняя граница
//
static vm_ins_result vm_rand(vm_struct *vm) {
...
//
// Обработчик инструкции get
// Получает код последней нажатой клавиши в r28
//
static vm_ins_result vm_get(vm_struct *vm) {
...
static void vm_init (vm_struct *vm) {
	...
	//забивка нулями VM_MEM_SIZE байт по адресу vm->memory
